...
describe('ErrorComponent', () => {
  ...
  it('should go to auth', async () => {

    await harness.navigateByUrl(`/error?error=${error}&error_description=${error_description}&reauthentication_location=${reauthentication_location}`, ErrorComponent);

    let hostDeWithQueryParam = harness.routeDebugElement!;


    // given (instead of when)

    windowServiceSpy.get.and.returnValues({
      location: {
        href: reauthentication_location
      }
    })

    // when
    harness.detectChanges();
    let goToAuthButton = hostDeWithQueryParam.query(By.css("button[mat-raised-button]")).nativeElement as HTMLButtonElement;
    goToAuthButton.click();
    harness.detectChanges();
